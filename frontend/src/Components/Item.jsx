...
import { ShopContext } from '../Context/ShopContext' // 👈 add this

export const Item = (props) => {
  const { wishlistItems, toggleWishlist } = useContext(WishlistContext);
  const { addToCart } = useContext(ShopContext); // 👈 add this
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 👇 add e.preventDefault() and addToCart call */}
        <button
          onClick={(e) => { e.preventDefault(); addToCart(props.id); }}
      ...
      <div className="pt-4 pb-2 flex flex-col items-start">
        <Link
          to={`/product/${props.id}`}
          onClick={() => window.scrollTo(0, 0)}
          className="font-sans font-normal text-base text-black tracking-wide hover:text-gray-500 transition-colors duration-300 mb-1 truncate w-full"
        >
          {props.name}
        </Link>
        <div className="flex items-center gap-3">
          <span className="font-sans text-base text-black font-medium">Rs {props.new_price}</span>
          <span className="font-sans text-sm text-gray-400 line-through font-light">Rs {props.old_price}</span>
        </div>
      </div>
    </div>
  )

    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto h-full px-6 md:px-16 flex items-center justify-between">
          ...
          <Link
            to="/"
            className="font-['Playfair_Display'] uppercase text-base md:text-2xl tracking-[0.15em] font-light text-black select-none hover:opacity-70 transition-opacity absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0"
          >
            ...
            {NAV_LINKS.map(({ label, to }) => {
              ...
              return (
                <Link
                  key={to}
                  to={to}
                  className={`text-base uppercase tracking-widest transition-colors py-2 relative group ${
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-20 px-6 md:hidden flex flex-col justify-between">
          <nav className="flex flex-col mt-4">
            <p className="text-[12px] uppercase tracking-widest text-gray-600 mt-6 mb-3">Menu</p>
            {NAV_LINKS.map(({ label, to }) => (
              <Link
                key={to}
                to={to}
                onClick={() => setMobileOpen(false)}
                className="text-3xl tracking-wide text-base block py-3 border-b border-gray-100"
              >
                {label}
              </Link>
            ))}

            <p className="text-[12px] uppercase tracking-widest text-gray-500 mt-8 mb-3">Account</p>
            <Link
              to="/wishlist"
              onClick={() => setMobileOpen(false)}
              className="text-2xl tracking-wide text-base block py-3 border-b border-gray-100"
            >
